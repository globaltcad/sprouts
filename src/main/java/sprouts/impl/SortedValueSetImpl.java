...
record SortedValueSetImpl<E>(
    ...
    private SortedValueSetImpl(
        ...
        this(type, ArrayItemAccess.of(type, false), comparator, root, new AtomicReference<>(null));
    }
    ...
    private static <E> boolean _recursiveEquals(@Nullable Node node1, @Nullable Node node2, Class<E> type, Comparator<E> comparator) {
        ...
        if (
            ...
            node1.elementsArray == node2.elementsArray &&
            (node1.left != node2.left || node1.right != node2.right)  // The only difference is somewhere deep down!
        ) {
            if ( !_recursiveEquals(node1.left, node2.left, type, comparator) ) {
                return false;
            }
            if ( !_recursiveEquals(node1.right, node2.right, type, comparator) ) {
                return false;
            }
            return true;
        } else {
