...
final class OrderedAssociationImpl<K, V> implements Association<K, V> {
    ...
    static class Frame {
        final Node node;
        int stage = 0;  // 0=left,1=values,2=right,3=done
        int idx = 0;
        Frame(Node n) { this.node = n; }
    }
    ...
    public Iterator<Pair<K, V>> iterator() {
        return new Iterator<Pair<K, V>>() {
            ...
            {
                if (_root.size() > 0) stack.push(new Frame(_root));
            }
            ...
            public boolean hasNext() {
                while (!stack.isEmpty()) {
                    ...
                    if (f.stage == 0) {
                        f.stage = 1;
                        if (f.node.left() != null) stack.push(new Frame(f.node.left()));
                    } else if (f.stage == 1) {
                        if (f.idx < _length(f.node.keysArray())) return true;
                        f.stage = 2;
                    } else if (f.stage == 2) {
                        f.stage = 3;
                        if (f.node.right() != null) stack.push(new Frame(f.node.right()));
                    } else {
                        stack.pop();
                    }
                }
            ...
            public Pair<K, V> next() {
                ...
                Frame f = stack.peek();
                K key = _getNonNullAt(f.idx, f.node.keysArray(), _keyType);
                V value = _getNonNullAt(f.idx, f.node.valuesArray(), _valueType);
                f.idx++;
                return Pair.of(key, value);
            }
        };
