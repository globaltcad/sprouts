...
class Linked_Association_Spec extends Specification
{
    ...
    def 'The `equals` and `hashCode` implementations of a linked Association works reliably after removing a large part of entries.'(
        Class<Object> keyType, Class<Object> valueType, List<Pair<Object, Object>> entries
    ) {
        reportInfo """
            Here we test the implementation of the `equals` and `hashCode` methods exhaustively.
    ...
    def 'The `equals` and `hashCode` implementations of a linked Association works reliably after a serious of modifications.'(
        Class<Object> keyType, Class<Object> valueType, List<Pair<Object, Object>> entries
    ) {
        reportInfo """
            Here we test the implementation of the `equals` and `hashCode` methods exhaustively.
        ...
        and :
            var originalAssociation = Association.betweenLinked(keyType, valueType).putAll(firstHalf)
    ...
    def 'The `update` method transforms existing values using a function while preserving the linked association structure.'() {
        reportInfo """
            The `update` method provides a safe way to modify existing values in a linked association
        ...
        given: 'A linked association representing product inventory with initial quantities'
            var inventory = Association.betweenLinked(String, Integer).putAll(
                Pair.of("apples", 10),
                Pair.of("oranges", 15),
                Pair.of("bananas", 20),
                Pair.of("grapes", 8)
    ...
    def 'The `update` method handles edge cases and exceptions appropriately on linked associations.'() {
        given: 'An association with various value types'
            var assoc = Association.betweenLinked(String, Object).putAll(
                Pair.of("number", 42),
                Pair.of("text", "hello"),
                Pair.of("list", [1, 2, 3])
            )
    ...
    def 'Use `updateAll` with a Tuple of keys to batch-update multiple values efficiently.'() {
        ...
        given: 'A linked association representing student grades'
            var grades = Association.betweenLinked(String, Integer).putAll(
                Pair.of("Alice", 85),
                Pair.of("Bob", 72),
                Pair.of("Charlie", 90),
                Pair.of("Diana", 68),
    ...
    def 'Use `updateAll` with a ValueSet of keys for type-safe batch updates on linked associations.'() {
        reportInfo """
        ...
        given: 'An association of product prices and a ValueSet of products on sale'
            var prices = Association.betweenLinked(String, Double).putAll(
    ...
    def 'Use `updateAll` with a Collection of keys for flexible batch updates on linked associations.'() {
        reportInfo """
        ...
        given: 'An association of website traffic statistics'
            var traffic = Association.betweenLinked(String, Integer).putAll(
                Pair.of("/home", 1500),
                Pair.of("/about", 800),
    ...
    def 'Use `updateAll` with a Stream of keys for efficient pipeline processing for linked associations.'() {
        reportInfo """
        ...
        given: 'A linked association of employee data'
            var employees = Association.betweenLinked(String, Map).putAll(
    ...
    def 'The update methods work correctly with large datasets and maintain performance.'(int size) {
        given: 'A large linked association and a subset of keys to update'
            var largeAssoc = Association.betweenLinked(Integer, Integer)
    ...
    def 'Update methods can be chained for complex transformation pipelines on linked associations.'() {
        reportInfo """
        ...
        given: 'An association representing a shopping cart'
            var cart = Association.betweenLinked(String, Map).putAll(
    ...
    def 'Update methods preserve linked association characteristics (linked).'() {
        given: 'A linked association'
            var linkedAssoc = Association.betweenLinked(String, Integer)
                .put("first", 1)
                .put("second", 2)
                .put("third", 3)

        when: 'We update values in the linked association'
            var updatedLinked = linkedAssoc.update("second") { value -> value * 2 }

        then: 'The linked characteristic is preserved'
            updatedLinked.isLinked()
            updatedLinked.keySet().toList() == ["first", "second", "third"]  // Order maintained
    }

    def 'Update methods handle complex value types and nested structures.'() {
        given: 'A linked association with complex nested values'
            var complexData = Association.betweenLinked(String, Map).putAll(
                Pair.of("user1", [
