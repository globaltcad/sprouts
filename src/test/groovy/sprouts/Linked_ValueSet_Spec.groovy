...
class Linked_ValueSet_Spec extends Specification {
    ...
    def 'You can remove items from a linked value set selectively, using `removeIf(Predicate)`.'(
        ...
        reportInfo """
            ...
            returned set.
        """
        given : 'We create a linked value set and a regular JDK set containing some test elements.'
            var valueSet = ValueSet.ofLinked(type, elements)
    ...
    def 'You can keep items in a linked value set selectively, using `retainIf(Predicate)`.'(
        Class<?> type, List<Object> elements, Predicate<Object> predicate
    ) {
        reportInfo """
    ...
    def 'The `equals` and `hashCode` implementations of a linked ValueSet works reliably after removing a large part of elements.'(
        ...
        when : 'We create two different `ValueSet` instances from the two scrambled list...'
            var set1 = ValueSet.ofLinked(type).addAll(scrambledEntries1)
            var set2 = ValueSet.ofLinked(type).addAll(scrambledEntries2)
    ...
    def 'The `equals` and `hashCode` implementations of a linked ValueSet works reliably after a serious of modifications.'(
        Class<Object> type, List<Object> entries
    ) {
        reportInfo """
            Here we test the implementation of the `equals` and `hashCode` methods exhaustively.
        ...
        and :
            var originalSet = ValueSet.ofLinked(type).addAll(firstHalf)
