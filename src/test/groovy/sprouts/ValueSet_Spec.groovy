...
class ValueSet_Spec extends Specification {
    ...
    def 'You can keep items in a value set selectively, using `retainIf(Predicate)`.'(
        Class<?> type, List<Object> elements, Predicate<Object> predicate
    ) {
        reportInfo """
    ...
    def 'The `equals` and `hashCode` implementations of a ValueSet works reliably after removing a large part of elements.'(
    ...
    def 'The `equals` and `hashCode` implementations of a ValueSet works reliably after a serious of modifications.'(
        Class<Object> type, List<Object> entries
    ) {
        reportInfo """
            Here we test the implementation of the `equals` and `hashCode` methods exhaustively.
        ...
        and :
            var originalSet = ValueSet.of(type).addAll(firstHalf)
